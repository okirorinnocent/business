...
st.markdown("""
    ...
    section[data-testid="stSidebar"] { background-color: #000814 !important; border-right: 1px solid rgba(255, 255, 255, 0.1); }
    ...
    .stButton>button { width: 100%; background-color: #007bff; color: white; border-radius: 8px; transition: 0.3s; }
    .stButton>button:hover { background-color: #0056b3; box-shadow: 0px 0px 15px rgba(0, 123, 255, 0.5); }
    </style>
...
if menu == "Inventory Hub":
    ...
    # --- THE "BULLETPROOF" DATA FETCH ---
    try:
        result = db.list_documents(DB_ID, COLL_ID)

        # Access documents safely without using .get() or []
        # In the new SDK, result is an object with a .documents attribute
        ...
        if all_docs:
            ...
            for doc in all_docs:
                # Extract data safely: check for .data attribute first
                d = getattr(doc, 'data', {})
                # Extract ID safely: check for .$id or .id
                ...
                with col_left:
                    ...
                    with st.popover("🗑️ Manage Inventory"):
                        to_delete = st.selectbox(
                            "Select item:", df['item_name'].tolist())
...
# --- 6. MODULE: CUSTOMER CHURN ---
    ...
    eng = st.slider("Engagement (%)", 0, 100, 50)
